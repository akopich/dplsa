...
import topicmodeling.DirichletDistribution
...
/** Defines Dirichlet symmetric prior on every topic
  ...
  */
class SymmetricDirichletTopicRegularizer(private[regulaizers] val alpha: Float) extends TopicsRegularizer
with MatrixInPlaceModification {
    private val dirichletDistribution = new DirichletDistribution(alpha)

    override def apply(topics: Array[Array[Float]]): Float =
        topics.foldLeft(0f)((sum, x) => sum + dirichletDistribution.logPDF(x))

    override def regularize(topics: Array[Array[Float]],
